...
from utils.torch_filter_module import FilterLayer
...
if __name__ == "__main__":
    ...
    if fb_array is not None:
        ...
        with torch.no_grad():
            torch_out_t = flayer(
                f_print1,  # seeded image
                freq_indices_map,  # already 1..100
    ...
    # filter the rest of the original fingerprint and plot
    for i in range(5):
        with torch.no_grad():
            torch_out_t = flayer(
    ...
    torch_out = torch_out_t.cpu().numpy()
    display_fprint(
        torch_out, animate=False, title="Torch FilterLayer Output After 5 Passes"
    )
    # for i in range(3):
    #     start = time.time()
    #     f_print1 = filter_image_withmap(
    #         f_print1=f_print1,
    #         filt_map=filt_map,
    #         freq_ind=freq_indices_map,
    #         orient_ind=orientation_indices_map,
    ...
    #         margin=margin,
    #         max_filter_size=max_filter_size,
    #         binarization_threshold=b_thresh,
    #     )
    #     filt_map = set_filter_area(
    #         filt_map=filt_map,
    #         H=height,
    ...
    #     print(f"filtering pass with map, iteration {i + 1} successful.")
    #     print(
    #         f"Time taken for filtering pass with map:{round(time.time() - start, 2)}s"
    #     )
    #     # display_fprint(f_print1, height, width, margin, animate=False, history=history)
    #     history.append(f_print1.copy())

    # for i in range(2):
    #     start = time.time()
    #     f_print1_alt = filter_with_math_gabor(
    #         f_print1=f_print1_alt,
    #         filt_map=filt_map,
    #         freq_ind=freq_indices_map,
    #         orient_ind=orientation_indices_map,
    #         filter_size_2d=filter_size_2D,
    ...
    #     )
    #     # display_fprint(f_print1, animate=False, history=history)
    #     history.append(f_print1_alt.copy())
    ...
    #     print(f"filtering pass without map, iteration {i + 1} successful.")
    #     print(
    #         f"Time taken for filtering pass without map:{round(time.time() - start, 2)}s"
    #     )
    #     history.append(f_print1.copy())
    #     # display_fprint(f_print1, height, width, margin, animate=False, history=history)

    # display_fprint(f_print1, animate=True, history=history)
    stride = 8
    # mgrid returns: 1st output (Y/Rows), 2nd output (X/Cols)
    ys, xs = np.mgrid[0:height:stride, 0:width:stride]

    U = np.cos(np.pi - orientation_map[::stride, ::stride])
    V = np.sin(np.pi - orientation_map[::stride, ::stride])

    plt.figure(figsize=(10, 10))  # Optional: make figure larger
    plt.imshow(
        np.clip((1 - f_print1 / 100.0) * 255, 0, 255).astype(np.uint8), cmap="gray"
    )
    plt.quiver(
    ...
    plt.axis("off")
    plt.title("Dense orientation field")
    plt.show()
