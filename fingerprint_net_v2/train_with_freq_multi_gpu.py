...
def train(
    ...
    for epoch in range(cfg.epochs):
        ...
        # NEW: CONSOLIDATE LOGS ON RANK 0
        if rank == 0:
        ...
        with torch.no_grad():
            for input in val_loader:
                ...
                pred = model(inputs)
                loss, _, _ = criterion(pred, spiral_phasor, target_c, target_f)
                val_loss += loss.item()

        # NEW: SYNC VAL LOSS ACROSS GPUS
        ...
        if rank == 0:
            ...
            if save_model and avg_val_loss < lowest_validation_loss:
                ...
                torch.save(checkpoint, "checkpoints/ckpt_parallel_1e-5.pth")
...
if __name__ == "__main__":
    ...
    cfg = TrainConfig(epochs=5000, batch_size=16, lr=1e-5)
