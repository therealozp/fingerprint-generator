"""
Model code from SwinUnet provided by chaineypung: https://github.com/chaineypung/Pytorch-Swin-Unet-V2
"""
...
class WindowAttention(nn.Module):
    ...
    def __init__(
        ...
        log_relative_position_index = torch.sign(relative_coords) * torch.log(
            1.0 + relative_coords.abs()
        )
        self.register_buffer("log_relative_position_index", log_relative_position_index)

        # Swin v2, small meta network, Eq.(3)
        self.cpb = Mlp_Relu(
            in_features=2,  # delta x, delta y
    ...
    def get_continuous_relative_position_bias(self, N):
        ...
        continuous_relative_position_bias = self.cpb(
            self.log_relative_position_index[:N, :N]
        )
...
class FingerprintSwinUnet(nn.Module):
    def __init__(
        self, config, img_size=224, num_classes=21843, zero_head=False, vis=False
    ):
        super(FingerprintSwinUnet, self).__init__()
        self.num_classes = num_classes
    ...
    def forward(self, x):
        ...
        # print("[DEBUG] Logits average magnitude:", logits.abs().mean())
